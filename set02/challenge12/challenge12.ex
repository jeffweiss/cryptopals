...
defmodule Challenge12Decryption do
  ...
  def c12(plaintext) do
    key = @key
    data = plaintext <> Base.decode64!(@text)
    |> Challenge12.pkcs7_padding
    Challenge12.encrypt_ecb(data, key, <<0>>)
  end
  ...
  def single_byte_decryption do
    ...
    encryption_mode = Challenge12.detect_block_mode(c12("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"), key_size)
    IO.puts "Detected #{encryption_mode} block mode"
    ciphertext_length = c12("") |> byte_size
